...
|---|---|---|---|---|
| 1 | [TASK-001](tasks/TASK-001.md) | DONE | 還原 runtime flow、Stage Contracts、Agent/Prompt、state mutation 與規格落差 | Completion Report 已驗證；不自動開始後續 Task |
| 2 | [TASK-002](tasks/TASK-002.md) | BLOCKED | 釐清並核准 canonical workflow／contract | TASK-001 Completion Report 驗證完成；unresolved decisions 有 owner |
| 3 | [TASK-003](tasks/TASK-003.md) | BLOCKED | 依核准 contract 規劃實作與 regression suite | TASK-002 驗收完成；變更範圍獲明確授權 |
...
See [UNRESOLVED_QUESTIONS.md](UNRESOLVED_QUESTIONS.md). Most material blockers are: canonical stage count/name, designated ideal CV oracle, definition of “actual produced CV”, runtime status of legacy prompt modes, and acceptance authority for Stage Contracts.
